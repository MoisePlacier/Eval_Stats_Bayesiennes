...
data <- list( c= data$c[1], m= data$m[1] , rm = data$rm[1] )
...
inits1 <- list(theta = 0.1, Nunif = 20000)

inits2 <- list( theta = 0.5, Nunif = 30000)

inits3 <- list( theta = 0.9, Nunif = 5000)
...
mcmc_sub <- window(mcmc, start = 15000, end = 16000)
chaine1 <- mcmc_sub[[1]]
plot(chaine1)
summary(chaine1)
...
var <- "theta"
mat <- do.call(cbind, lapply(mcmc_sub, function(ch) ch[, var]))
...
legend(
  "topright",
  legend = paste("Chain", seq_len(ncol(mat))),
  col = seq_len(ncol(mat)),
  lwd = 1
)
# --------------------------------------------------------
