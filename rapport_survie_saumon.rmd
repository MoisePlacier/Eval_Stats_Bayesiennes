...
$$\log(N_{\text{juv,obs}}(t-1)) = \log(N_{\text{juv}}(t-1)) + \varepsilon(t), \quad \varepsilon(t) \sim \mathcal{N}(0, \tau^{-1}) \quad <=> \quad N_{\text{juv,obs}}(t-1) \sim \text{LogNormal}(\log N_{\text{juv}}(t-1), \tau) $$
...
### Distribution du taux de survie annuel surv[i]

![Distribution du taux de survie annuel](img/Distrib_surv.png) 
...
![Lois jointes éstimées du taux de survie et du nombre de juvénils](img/Densite_marginale_surv_njuv.png){ width=40% }

Le graphique illustre la loi conjointe estimée du nombre de juvéniles et de leur taux de survie. On y observe une corrélation négative : lorsque le nombre de juvéniles augmente, le taux de survie tend à diminuer. Cette relation résulte de la structure même du modèle, où un nombre plus élevé de juvéniles doit s’accompagner d’un taux de survie plus faible pour que la vraisemblance d’observer les valeurs mesurées reste cohérente.

Même si ici cette corrélation négative est en grande partie une conséquence de la construction du modèle, il reproduit ce que l’on pourrait interpréter comme un effet de densité ou de compétition : à mesure que le nombre de juvéniles augmente, la survie individuelle diminue.

Pour évaluer si l’effet densité / compétition pourrait expliquer la variabilité interannuelle du taux de survie des juvéniles, nous avons tracé ce graphique sous forme de scatter plot représentant les estimations issues des simulations MCMC. Chaque nuages de points représente les densités estimées annuelles de juvéniles et des taux de survie associés.

![Scatter plot des estimations MCMC : taux de survie x log(nombre de juvénils)](img/surv_njuv.png){width=80%}
...
![Distribution des abondances annuelles estimées et distribution de la survie](img/Hell_plot.png){width=80%}

Enfin sur ce graphique, on observe que la distribution estimée du nombre de juvéniles de l'année précédente (Njuv à l'année n‑1) se situe parfois en dessous de la distribution du nombre total d'individus à l'année n $N(t)$, comme c'est le cas pour l'année 9. Cette différence s'explique probablement par l'hypothèse simplificatrice sur le paramètre pSm1, que l'on considère ici comme fixe, alors qu'en réalité il présente une variabilité.
